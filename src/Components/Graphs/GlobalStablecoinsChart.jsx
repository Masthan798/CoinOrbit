                ...
                <div className="flex items-center bg-white/5 p-0.5 sm:p-1 rounded-md w-full sm:w-auto overflow-x-auto no-scrollbar">
                    {timeframes.map((tf) => (
                        <button
                            ...
                            className={`px-2.5 sm:px-3 py-1.5 rounded-md text-[9px] sm:text-[10px] font-bold transition-all ${timeframe.label === tf.label ? 'bg-green-500 text-white shadow-lg shadow-green-500/20' : 'text-gray-400 hover:text-white'}`}
                        >
                            {tf.label}
                        </button>
                    ))}
